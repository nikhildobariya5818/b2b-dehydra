...
type ProductReferenceLayoutProps = { product: { name: string; description: string; imageUrl?: string | null; specifications?: unknown; category?: string | null }; };
...
export function ProductReferenceLayout({ product }: ProductReferenceLayoutProps) {
  ...
  return <>
    <section className="reference-hero">
      <div className="container reference-hero-grid"><div><p className="eyebrow">Export quality grade</p><h1>{product.name}</h1><p>{product.description}</p><div className="button-row"><Link className="button dark" href={`/contact?product=${encodeURIComponent(product.name)}&intent=brochure`}><Icon name="download" size={16}/> Download brochure</Link><Link className="button accent" href={`/contact?product=${encodeURIComponent(product.name)}&intent=quote`}>Request B2B quote</Link></div></div><div className="reference-hero-image"><Image src={image} alt={product.name} fill priority sizes="(max-width: 800px) 100vw, 50vw" className="cover-image"/></div></div>
    </section>
    <section className="section container reference-overview"><div><div className="reference-main-image"><Image src={image} alt={`${product.name} production`} fill sizes="50vw" className="cover-image"/></div><div className="reference-thumbs"><Image src="/images/potato-2.jpg" alt="Laboratory testing" fill className="cover-image"/><Image src="/images/potato-3.jpg" alt="Product texture" fill className="cover-image"/><Image src="/images/potato-4.jpg" alt="Export packaging" fill className="cover-image"/></div></div><div className="reference-side"><div className="reference-card"><h3>Technical Overview</h3>{[["Purity Level", "99.8% Minimum"], ["Moisture Content", "Max 8.0%"], ["Shelf Life", "18 Months"]].map(([a,b]) => <div className="reference-row" key={a}><span>{a}</span><strong>{b}</strong></div>)}<div className="reference-row"><span>Certifications</span><CertificationLogos items={["ISO 22000", "HACCP", "GMP"]}/></div><Link className="button outline full" href={`/contact?product=${encodeURIComponent(product.name)}&intent=sample`}><Icon name="download" size={15}/> Request sample</Link></div><div className="reference-card muted"><h3>Primary Applications</h3>{applications.map(([title, text]) => <div className="application-item" key={title}><Icon name="box" size={17}/><div><strong>{title}</strong><p>{text}</p></div></div>)}</div></div></section>
