...
export function ProductReferenceLayout({ product }: ProductReferenceLayoutProps) {
  ...
  return <>
    <section className="reference-hero">
      <div className="container reference-hero-grid"><div><p className="eyebrow">Export quality grade</p><h1>{product.name}</h1><p>{product.description}</p><div className="button-row"><a className="button dark" href="#specifications"><Icon name="download" size={16}/> Download specification PDF</a><Link className="button accent" href="/contact">Request B2B quote</Link></div></div><div className="reference-hero-image"><Image src={image} alt={product.name} fill priority sizes="(max-width: 800px) 100vw, 50vw" className="cover-image"/></div></div>
    </section>
